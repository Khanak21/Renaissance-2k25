...
function StickyNavbar() {
  ...
  return (
    ...
    // navbar component
    <nav
      className={`w-full text-custom-light flex flex-row justify-between z-[1000] transition duration-600 ${
        navScroll ? "shadow-xl bg-custom-primary" : "bg-custom-primary"
      }`}
    >
      {/* logo  */}
      <div
        ...
        <ul className="flex flex-row items-center justify-center text-sm font-light text-custom-secondary">
          {navItems.map((item) => (
            <Link
              ...
              className="mx-2 px-1 lg:mx-4 lg:px-2 relative  before:content-[''] before:absolute before:bg-custom-warm before:h-[3px] before:w-0 before:left-0 before:bottom-[-8px] before:transition-[0.3s] before:duration-300 hover:before:w-full hover:text-custom-warm"
            >
